...
public class Answer3 {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			fw = new FileWriter("c:/ecount/결과파일/ans3.txt");
			...
			int sum=0;
			//거래등급이 20이상인 {거래처코드, 거래점수+추가포인트}
			int temp[][] = new int[end-start+1][2];
			for(int i=0; i<end-start+1; i++) {
				
				//거래등급 20 이상이면
				if(Integer.parseInt(table[i][7].trim())>=20) { //공백제거하여 형변환
					...
					temp[i][1] = Integer.parseInt(table[i][10].trim()) + point; //거래점수+추가포인트 합
					sum += temp[i][1];
				}
			}
